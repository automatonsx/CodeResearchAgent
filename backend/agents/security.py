"""Security Agent — secrets, injection, unsafe calls grounded in best_practices corpus.

Ground truth: ruff's bandit (S) rules + semgrep when available + generic pattern scanner.
Citations come from the ChromaDB grounding node (best_practices.json) — no web search.
"""
...
def security_node(state: ReviewState) -> ReviewState:
    """Run deterministic security scanners, then structure findings via LLM."""
